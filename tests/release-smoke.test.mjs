...
const pythonCandidates = process.platform === 'win32'
  ? [['python', []], ['py', ['-3']], ['python3', []]]
  : [['python3', []], ['python', []]];
const python = pythonCandidates.find(([command, args]) => spawnSync(command, [...args, '--version']).status === 0);
assert.ok(python, 'Python 3 is required for the local release smoke test');
const child = spawn(python[0], [...python[1], 'server.py'], { cwd: root, env: { ...process.env, PORT:String(port), HOST:'127.0.0.1' }, stdio:['ignore', 'pipe', 'pipe'] });
let serverError = '';
...
try {
  ...
  assert.equal(ready, true, `local server did not start${serverError ? `: ${serverError.trim()}` : ''}`);
  ...
  assert.match(health.headers.get('x-pathwise-request-id') || '', /^pw_srv_[a-f0-9]{16}$/);
  assert.equal(typeof (await health.json()).configured, 'boolean');
  const correlated = await fetch(`${base}/api/health`, { headers:{ 'X-Pathwise-Request-Id':'pw_test_correlation' } });
  assert.equal(correlated.headers.get('x-pathwise-request-id'), 'pw_test_correlation');
