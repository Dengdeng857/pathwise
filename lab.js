...
function show(index) { const role = choices[index]; const gap = (plan?.gaps || [])[index] || '一条可验证的真实成果'; insight.innerHTML = `<span class="kicker">AI READOUT / 0${index + 1}</span><strong>${escapeHtml(role.title)}</strong><p>当前匹配度 ${Number(role.match || 0)}。最值得先验证的是：${escapeHtml(gap)}。</p><div class="lab-micro"><span>7 天验证</span><b>${index === 0 ? '拆解岗位要求，确认你是否喜欢这类问题' : index === 1 ? '完成一个可展示项目，并获得外部反馈' : '做一次深挖面试，测试能力上限'}</b></div><a class="text-button" href="career.html#route">开始一次验证 →</a>`; }
